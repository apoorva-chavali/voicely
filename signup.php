...
session_start();
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($user_name) && !empty($password) && !is_numeric($user_name) && !empty($email) && !empty($age) && !empty($gender) && mysqli_num_rows($query_run)==0)
		{

			//save to database
			$query = "insert into users (user_name,email,age,gender,password) values ('$user_name','$email','$age','$gender','$password')";
...
<body>
	<div id="box">
		...
		<form method="post">
			...
			<input id="text" type="text" name="age" placeholder=" age" required><br><br>
			...
			<div><button type="button" id="button"><a href="login.php">Already a user</a></button><div>
			</div>
